...
const Share2 = (p) => <Ico {...p}><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.6" y1="10.5" x2="15.4" y2="6.5"/><line x1="8.6" y1="13.5" x2="15.4" y2="17.5"/></Ico>;
...
const SUPA_ANON = process.env.REACT_APP_SUPABASE_PUBLISHABLE_KEY;
...
// Token para links públicos (cotización compartible) — más largo y aleatorio que genId,
// para que adivinarlo por fuerza bruta no sea viable.
const genShareToken = () => {
...
// ─── ROOT APP ─────────────────────────────────────────────────────────────────
// ─── VISTA PÚBLICA DE COTIZACIÓN (link compartible, sin login) ──────────────
// Ruta: /c/{shareToken}. Lee UNA sola cotización por su token vía una función
// RPC de Supabase (get_quote_by_token) que no expone el resto de la tabla —
// no hay forma de "listar" cotizaciones ajenas, solo de leer la que tenés el link.
function PublicQuoteView({token}) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if(err || !data || !data.length) { setError(true); setLoading(false); return; }
        setQuote(mapQuote(data[0]));
      } catch(e) {
        setError(true);
      } finally {
        setLoading(false);
  ...
  return (
    <div style={{background:"#e8e2d5",minHeight:"100vh",fontFamily:"'Segoe UI',Arial,sans-serif"}}>
      ...
      {lightbox && (
        <div onClick={()=>setLightbox(null)} style={{position:"fixed",inset:0,background:"#000d",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:20}}>
          <button onClick={(e)=>{e.stopPropagation();setLightbox(null);}} style={{position:"absolute",top:20,right:20,width:40,height:40,borderRadius:"50%",background:"#fff2",color:"#fff",border:"none",fontSize:22,cursor:"pointer"}}>×</button>
          <img src={lightbox} alt="" style={{maxWidth:"100%",maxHeight:"80vh",borderRadius:10,boxShadow:"0 10px 40px #0008"}}/>
          <div style={{position:"absolute",bottom:24,left:0,right:0,textAlign:"center",color:"#fff9",fontSize:12}}>Tocá afuera de la imagen para cerrar</div>
        </div>
      )}
    </div>
  );
...
function AppInner() {
  // ── LINK PÚBLICO DE COTIZACIÓN (/c/token) — se resuelve ANTES que cualquier
  // lógica de sesión/auth, así el cliente no necesita loguearse para verla.
  const publicToken = useMemo(() => {
    const m = window.location.pathname.match(/^\/c\/([a-zA-Z0-9]+)\/?$/);
    return m ? m[1] : null;
  }, []);
  if(publicToken) return <PublicQuoteView token={publicToken}/>;
...
function ProductSelector({products,cart,setCart,isMobile,promos=[],loteMode=false,orders=[],sortByPopularity=false}) {
  ...
  const addC=p=>setCart(c=>{
    ...
    return [...c,{pid:p.id,id:p.id,cartKey:p.id,qty:qtyInicial,price:p.salePrice,name:p.name,disc,promoLabel:label,imageUrl:p.imageUrl||"",...encargueFields(p,qtyInicial)}];
  });
