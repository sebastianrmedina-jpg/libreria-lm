...
function ExcelPanel({products,setProducts}) {
  ...
  const parseExcel = (file) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        // Fallback posición fija según estructura real del Excel:
        // A(0)=Código B(1)=Descripción C(2)=P.IVA D(3)=P.Oferta E(4)=Fecha F(5)=P.Final
  ...
  const applyImport = async () => {
    ...
    // Precio de venta: col F (precioFinal) tiene prioridad, sino D (oferta), sino C (IVA)
    // Precio de costo: col D (oferta) si existe, sino col C (IVA)
    ...
    preview.all.forEach(row=>{
      ...
      const precioCosto = resolvePrice(row); // D si existe, sino C
      if(idx>=0){
        // Precio de venta: col F (precioFinal) tiene prioridad, sino D, sino C
        ...
        // Precio de costo: D si existe, sino C
